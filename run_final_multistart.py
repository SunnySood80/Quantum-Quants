"""
Final multistart runner — runs N independent starts on the chosen best grid cell
(risk_penalty=3.0, cardinality_penalty=0.1) and saves the final best as 'final'.
"""
import os
import pickle
import time
...
# Final run parameters (chosen best grid cell)
risk_penalty = 3.0
...
# QAOA optimizer class (re-using the robust COBYLA implementation)
class QAOAPortfolioOptimizer:
    def __init__(self, Q, n_qubits, p=3, max_iter=150, simulator=None, shots_eval=1000, shots_final=10000):
        self.Q = Q
...
for s in range(nstarts):
    ...
    # Save per-start
    portfolio['portfolio_df'].to_csv(os.path.join(OUTDIR, f'qaoa_final_start{s+1}_portfolio.csv'), index=False)
    pd.DataFrame([metrics]).to_csv(os.path.join(OUTDIR, f'qaoa_final_start{s+1}_metrics.csv'), index=False)
...
# Save summary
pd.DataFrame(summary).to_csv(os.path.join(OUTDIR, 'qaoa_final_multistart_summary.csv'), index=False)

if best_artifact is not None:
    best_artifact['portfolio']['portfolio_df'].to_csv(os.path.join(OUTDIR, 'qaoa_final_best_portfolio.csv'), index=False)
    pd.DataFrame([best_artifact['metrics']]).to_csv(os.path.join(OUTDIR, 'qaoa_final_best_metrics.csv'), index=False)
    print('\nFINAL BEST START:', best_artifact['start'])
    print('Final Best Sharpe:', best_artifact['metrics']['sharpe_ratio'])
else:
    print('No successful starts')

elapsed = time.time() - start_time
print(f'Completed {len(summary)} starts in {elapsed:.1f}s')
print('Final artifacts saved to results/qaoa_final_best_*')
