"""
2D parameter sweep: risk_penalty x cardinality_penalty grid.
Saves per-cell portfolios and a summary CSV `qaoa_2d_param_sweep_summary.csv`.
"""
import os
import pickle
import time
...
for rp in risk_list:
    for cp in card_list:
        print('\n' + '='*70)
        print(f'Running rp={rp}, cardinality_penalty={cp}')
        Q_df = build_qubo(mu_latent, Sigma_latent, risk_penalty=rp,
                          cardinality_penalty=cp, target_cardinality=target_cardinality)
        Q = Q_df.values
        ...
        try:
            ...
            print(f'  QAOA failed for rp={rp}, cp={cp}: {e}')
            continue
        ...
        # Save per-cell
        safe_rp = str(rp).replace('.', '_')
        safe_cp = str(cp).replace('.', '_')
        portfolio['portfolio_df'].to_csv(os.path.join(OUTDIR, f'qaoa_rp{safe_rp}_cp{safe_cp}_portfolio.csv'), index=False)
        pd.DataFrame([metrics]).to_csv(os.path.join(OUTDIR, f'qaoa_rp{safe_rp}_cp{safe_cp}_metrics.csv'), index=False)
...
df_summary.to_csv(os.path.join(OUTDIR, 'qaoa_2d_param_sweep_summary.csv'), index=False)

elapsed = time.time() - start_time
print('\n2D sweep complete in %.1f seconds' % elapsed)
print('Summary saved to results/qaoa_2d_param_sweep_summary.csv')
